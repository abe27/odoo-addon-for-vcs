...
class OrderHeader(models.Model):
    ...
    order_date = fields.Date(
        string="Date", defualt=lambda self: fields.Date.today())
    name = fields.Char(size=15, string="Order No.", required=True)
    partner_id = fields.Many2one(
        "res.partner", string="Partner", required=True)
    item_count = fields.Integer(string="Item", defualt="0")
    vat_total = fields.Float(string="Vat.", defualt="0.0")
    order_step = fields.Many2one('vcsgroup.order_step', string="Step")
    remark = fields.Text(string="Remark", defualt="-")
    is_approve = fields.Selection([("0", "Open"), ("1", "In Process"), (
        "2", "Approved"), ("3", "Completed"), ("4", "Cancel")], string="Status")
    is_sync = fields.Boolean(string="Is Sync", defualt=False)
...
class OrderDetail(models.Model):
    ...
    quantity = fields.Float(string="Quantity", defualt="0.0")
    unit_id = fields.Many2one( 'vcsgroup.unit', string="Product", required=True)