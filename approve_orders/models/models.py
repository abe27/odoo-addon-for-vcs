...
class OrderHeader(models.Model):
    ...
    def _update_totals(self):
        for record in self:
            ...
            for line_id in record.line_ids:
                vat += (float(line_id['quantity']) *
                        float(line_id['product_id']['price']))
    ...
    def create(self, data):
        ...
        if data['order_type_id']:
            book = self.env['vcsgroup.booking'].search(
                [('id', '=', data['order_type_id'])])
            if len(book.prefix) > 1:
                book_prefix = book.prefix

        ctnRecord = self.env['approve_orders.order_header'].search_count(
            [('name', 'like', f"{book_prefix}{dte.strftime('%Y%m')[3:]}")])
    ...
    def onchange_line_ids(self):
        ...
        for r in self.line_ids:
            vatCount += (float(r['quantity']) *
                         float(r['product_id']['price']))
            if len(_id) > 0:
                try:
                    ...
                    print(
                        f"Found Duplicate: {_id.index(r['product_id']['id'])}")
